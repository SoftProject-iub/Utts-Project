...
def admin():
    ...
    try:
        ...
        return render_template('admin.html', buses=buses[0], users=users[0], routes=routes[0], feedback=feedback[0])
    finally:
        cursor.close()
        conn.close()
...
def add_bus():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return redirect(url_for('bus_management'))
        finally:
            cursor.close()
            conn.close()
